...
public class controlUI extends Activity implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    private short[] dataSend1 = new short[4];
    private short[] dataSend2 = new short[4];
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (dataSend1[0] != dataSend1[2] || dataSend1[1] != dataSend1[3]) {
                    s[3] = 'A';
                    s[4] = (byte) ((dataSend1[0] & 0xff00) >> 8);
                    s[5] = (byte) (dataSend1[0] & 0x00ff);
                    Log.d("bletrack",String.valueOf(s[4]));
                    Log.d("bletrack",String.valueOf(s[5]));
                    Log.d("bletrack",String.valueOf(dataSend1[0]));
                    s[6] = (byte) (dataSend1[1] & 0x00ff);
                    //String s="AT+A"+String.valueOf(dataSend1[0])+'\t'+String.valueOf(dataSend1[1])+"\r\n";
                    sendString(s);
                }
                if (dataSend2[0] != dataSend2[2] || dataSend2[1] != dataSend2[3]) {
                    s[3] = 'B';
                    s[4] = (byte) ((dataSend2[0] & 0xff00) >> 8);
                    s[5] = (byte) (dataSend2[0] & 0x00ff);
                    s[6] = (byte) (dataSend2[1] & 0x00ff);
                    //String s="AT+B"+String.valueOf(dataSend2[0])+'\t'+String.valueOf(dataSend2[1])+"\r\n";
                    sendString(s);
                }
        ...
        wheelView_01.setOnWheelViewMoveListener(
                new WheelView.OnWheelViewMoveListener() {
                    ...
                    public void onValueChanged(int angle, int distance) {
                        ...
                        dataSend1[0] = (short) angle;
                        dataSend1[1] = (short)distance;
                    }
                }, 100L);
        wheelView_02.setOnWheelViewMoveListener(
                new WheelView.OnWheelViewMoveListener() {
                    ...
                    public void onValueChanged(int angle, int distance) {
                        ...
                        dataSend2[0] = (short)angle;
                        dataSend2[1] = (short)distance;
                    }
                }, 100L);
    }
...
//                        BluetoothLeService.EXTRA_DATA));
    ...
    private void sendString(byte[] s) {
        if (!mFindService)
            return;
        target_chara.setValue(s);
        //调用蓝牙服务的写特征值方法实现发送数据
